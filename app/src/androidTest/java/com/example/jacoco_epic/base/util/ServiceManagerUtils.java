...
//        ReflectionUtils.setStaticField(Class.forName(CLASS_ServiceManager), "sCache", sCache);
    ...
    public static IBinder mActivityManagerBinder = null;
    /**
    ...
    public static IBinder hookActivityManager(IServiceInvokeCallBack iServiceInvokeCallBack) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException, NoSuchFieldException {
        if (mActivityManagerBinder != null) {
            resetActivityManager(mActivityManagerBinder);
            mActivityManagerBinder = null;
        }
        if (Build.VERSION.SDK_INT <= 25) {
            mActivityManagerBinder = hookActivityManager7_0(iServiceInvokeCallBack);
        } else {
            mActivityManagerBinder = hookActivityManager8_0(iServiceInvokeCallBack);
        }
        return mActivityManagerBinder;
    }
    public static IBinder hookActivityManager7_0(IServiceInvokeCallBack iServiceInvokeCallBack) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException, NoSuchFieldException {
        //1.反射拿到service对象所有方法实现
        ...
        //2.2 创建Ibinder动态代理，实现invoke方法
        IBinder iBinder = (IBinder) ReflectionUtils.invokeStaticMethod(CLASS_ServiceManager, "getService", new Class[] {String.class}, new Object[] {"activity"});
        InvocationHandler serviceBinderHook = new ServiceBinderHook(iBinder, iServiceManagerHook);
        IBinder iBinderHook = (IBinder) Proxy.newProxyInstance(serviceBinderHook.getClass().getClassLoader(), new Class[]{IBinder.class}, serviceBinderHook);
        HashMap<String, IBinder> sCache = (HashMap<String, IBinder>) ReflectionUtils.getStaticField(Class.forName(CLASS_ServiceManager), "sCache");
    ...
    public static IBinder hookActivityManager8_0(IServiceInvokeCallBack iServiceInvokeCallBack) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException, NoSuchFieldException {
        //1.反射拿到service对象所有方法实现
        Object iActivityManager = ReflectionUtils.invokeStaticMethod(ActivityManager.class.getName(), "getService", null, null);
        //2.1 创建IServiceManager动态代理，实现invoke方法
        InvocationHandler serviceProxyHook = new ServiceProxyHook(iActivityManager, iServiceInvokeCallBack);
        Object iServiceManagerHook =  Proxy.newProxyInstance(serviceProxyHook.getClass().getClassLoader(), iActivityManager.getClass().getInterfaces(), serviceProxyHook);
        //2.2 创建Ibinder动态代理，实现invoke方法
        ...
        ReflectionUtils.setStaticField(Class.forName(CLASS_ServiceManager), "sCache", sCache);
        //4. 将ActivityManagerNative中的gDefault单例重置，使其重新去ServiceManager中获取
        Object singleton = ReflectionUtils.getStaticField(ActivityManager.class.getName(), "IActivityManagerSingleton");
        ReflectionUtils.setPrivateFieldAnyway(singleton, "mInstance", null);
        return iBinder;
    }
